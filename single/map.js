...
function loadMap(file, AInum){
	$.getJSON(file, function(data) {
		...
		game_data.AIid=data.AI;
		game_data.AItype=data.AItype;
		game_data.AI=[];

		for(var i=0; i<game_data.AIid.length; i++){
			var new_ai = new AI(game_data.AItype[i], game_data.AIid[i]);
			game_data.AI.push(new_ai);
		}
